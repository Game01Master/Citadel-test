...
export default function App() {
  ...
  const getBaseStrength = (troopName) => {
    ...
    return Number(t?.baseStrength ?? t?.base_strength ?? t?.strength ?? t?.base ?? 0);
  };
  ...
  const getBaseHealth = (troopName) => {
    ...
    const exact = canon.get(normName(troopName)) || troopName;
    const t = troopByName.get(exact);
    return Number(t?.baseHealth ?? t?.base_health ?? t?.health ?? t?.hp ?? 0);
  };
  ...
  const isFirstStrikerTroop = (troopName) => {
    ...
    return list.some(n => (canon.get(normName(n)) || n) === exact);
  };
  ...
  const targets = useMemo(() => cit ? (mode === MODE_WITH ? cit.m8m9Targets : cit.normalTargets) : null, [cit, mode]);
  ...
  const poolAll = useMemo(() => {
    ...
    const out = raw.map(r => canon.get(normName(r))).filter(Boolean);
    return Array.from(new Set(out));
  }, [mode, canon]);

  const wallKillerPool = useMemo(() => {
    const out = WALL_KILLER_NAMES_RAW.map(r => canon.get(normName(r))).filter(Boolean);
    return Array.from(new Set(out));
  }, [canon]);
  ...
  const firstAllowed = useMemo(() => {
    ...
    const out = rawList.map(r => canon.get(normName(r))).filter(c => c && allowedSet.has(normName(c)));
    return Array.from(new Set(out));
    ...
    if (idx >= 2) {
      ...
      if (first && picked && isFirstStrikerTroop(picked)) {
        ...
        if (pickedS > firstS || pickedH > firstH) {
          setWarningMsg(`${STRIKER_LABELS[idx]} (${picked}) has higher BASE stats than your First striker (${first}). Choose a stronger First striker!`);
          setTroopAt(idx, "");
          return;
        }
      }
  ...
  const setBonusAt = (idx, v) => {
    ...
    if (g) {
      ...
      setStrikerBonusPct((prev) => {
        const next = [...prev];
        for (let i = 0; i < strikerTroops.length; i++) { if (getBonusGroup(strikerTroops[i]) === g) next[i] = v; }
        return next;
      });
    } else {
  ...
  const firstDeaths = useMemo(() => {
    ...
    const effHealth = (troop ? toNum(troop.health) : 0) * (1 + toNum(firstHealthBonusPct) / 100);
  ...
  const wallKiller = useMemo(() => {
    ...
    const dmgPerTroop = (troop ? toNum(troop.strength) : 0) * (1 + effBonus / 100) * 20;
  ...
  const perStriker = useMemo(() => {
    ...
    return STRIKER_LABELS.map((label, idx) => {
      ...
      const dmgPerTroop = (troop ? toNum(troop.strength) : 0) * (1 + effBonus / 100);
  ...
  const showResults = () => {
    ...
    perStriker.forEach(s => { if (s?.troopName && s?.requiredTroops) add(s.troopName, s.requiredTroops); });
    const ordered = RESULT_ORDER.filter(name => counts.has(normName(name))).map(name => ({ troop: name, required: counts.get(normName(name)) }));
    ...
    <div style={{ width: "100%", minHeight: "100vh", color: theme.text, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ width: "100%", maxWidth: 600, margin: "0 auto", padding: "20px 16px", position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", marginBottom: 30 }}>
          <div style={{ fontWeight: 800, fontSize: 32, background: `linear-gradient(to bottom, #fff, ${theme.accent})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", fontFamily: "'Cinzel', serif", textTransform: "uppercase", letterSpacing: 2, filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.5))" }}>Citadel Calculator</div>
          <div style={{ marginTop: 6, fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: theme.subtext, opacity: 0.75 }}>by GM</div>
        </div>
        ...
        <div style={{ display: "grid", gap: 16, paddingBottom: 100 }}>
          <Card title="⚙️ Setup" theme={theme}>
            ...
            <div style={{ display: "grid", gap: 16 }}>
              ...
              <button onClick={resetSelections} style={{ width: "100%", padding: "12px 16px", borderRadius: 10, border: `1px solid ${theme.danger}`, background: "rgba(255, 77, 77, 0.15)", color: "#ff6b6b", fontWeight: 700, fontSize: 14, cursor: "pointer", marginTop: 8 }}>Reset Troops Selection</button>
            </div>
          </Card>

          <Card title="🛡️ Wall Killer" theme={theme}>
            <div style={{ display: "grid", gap: 16 }}>
              <TroopPicker label="Select Troop" value={wallKillerTroop} options={wallKillerPool} onChange={(v) => { setWallKillerTroop(v); setCalcOutput(null); setResultsOpen(false); }} theme={theme} inputStyle={inputStyle} />
              <label style={{ display: "grid", gap: 8 }}><span style={{ color: theme.subtext, fontWeight: 600, fontSize: 13, textTransform: "uppercase" }}>Strength Bonus (%)</span><input type="number" step="any" inputMode="decimal" placeholder="0" value={wallKillerBonusPct} onChange={(e) => { setWallKillerBonusPct(e.target.value); setCalcOutput(null); setResultsOpen(false); }} style={inputStyle} onFocus={(e) => e.target.select()} /></label>
              <div style={{ background: "rgba(0,0,0,0.42)", padding: "12px 16px", borderRadius: 10, border: `1px solid ${theme.border}`, boxShadow: theme.goldGlow }}><Row label="Effective Bonus" value={`${fmtInt(wallKiller.effBonus)}%`} theme={theme} accent /><Row label="Required Troops" value={fmtInt(wallKiller.requiredTroops)} theme={theme} accent /></div>
            </div>
          </Card>

          {perStriker.map((s) => (
            <Card key={s.idx} title={`${s.idx + 1}. ${s.label}`} theme={theme}>
              <div style={{ display: "grid", gap: 16 }}>
                <TroopPicker label="Select Troop" value={strikerTroops[s.idx]} options={optionsForIdx(s.idx)} onChange={(v) => handleTroopChange(s.idx, v)} theme={theme} inputStyle={inputStyle} />
                {s.idx === 0 && (<label style={{ display: "grid", gap: 8 }}><span style={{ color: "#ff8a8a", fontWeight: 700, fontSize: 13, textTransform: "uppercase" }}>Health Bonus (%)</span><input type="number" step="any" inputMode="decimal" placeholder="0" value={firstHealthBonusPct} onChange={(e) => { setFirstHealthBonusPct(e.target.value); setCalcOutput(null); setResultsOpen(false); }} style={{...inputStyle, borderColor: "rgba(255, 138, 138, 0.4)"}} onFocus={(e) => e.target.select()} /></label>)}
                <label style={{ display: "grid", gap: 8 }}><span style={{ color: "#80d8ff", fontWeight: 700, fontSize: 13, textTransform: "uppercase" }}>Strength Bonus (%)</span><input type="number" step="any" inputMode="decimal" placeholder="0" value={strikerBonusPct[s.idx]} onChange={(e) => setBonusAt(s.idx, e.target.value)} style={{...inputStyle, borderColor: "rgba(128, 216, 255, 0.4)"}} onFocus={(e) => e.target.select()} /></label>
                <div style={{ background: "rgba(0,0,0,0.42)", padding: "12px 16px", borderRadius: 10, border: `1px solid ${theme.border}`, boxShadow: theme.goldGlow }}><Row label="Effective Bonus" value={`${fmtInt(s.effBonus)}%`} theme={theme} accent /><Row label="Required Troops" value={fmtInt(s.requiredTroops)} theme={theme} accent />{s.idx === 0 && (<div style={{ marginTop: 8, paddingTop: 8, borderTop: `1px solid ${theme.borderSoft}` }}><Row label="Citadel First Strike Losses" value={fmtInt(firstDeaths)} theme={theme} /></div>)}</div>
              </div>
            </Card>
          ))}
        </div>

        <div style={{ position: "fixed", left: 0, right: 0, bottom: 8, padding: 16, zIndex: 99 }}><div style={{ width: "100%", maxWidth: 600, margin: "0 auto" }}><button onClick={showResults} style={{ width: "100%", padding: "16px", borderRadius: 12, border: "none", background: theme.btnBg, color: theme.btnText, fontWeight: 900, letterSpacing: 1, fontSize: 18, fontFamily: "'Cinzel', serif", boxShadow: `0 0 20px rgba(197, 160, 89, 0.4)`, cursor: "pointer" }}>CALCULATE</button></div></div>
        ...
        <Modal open={helpOpen} title="ℹ️ Instructions & Help" onClose={() => setHelpOpen(false)} theme={theme}><div style={{ color: theme.text, lineHeight: 1.6, fontSize: 15, display: "grid", gap: 20 }}><div><div style={{ fontWeight: 800, marginBottom: 8, fontSize: 18, color: theme.accent }}>🎯 Goal</div><div style={{ color: theme.subtext }}>Use the correct troops and bonuses to minimize losses when attacking a Citadel.</div></div><div><div style={{ fontWeight: 800, marginBottom: 8, fontSize: 18, color: theme.danger }}>❗ Most Important Rule</div><div style={{ color: theme.subtext, borderLeft: `4px solid ${theme.danger}`, paddingLeft: 12 }}>Maximize <b>First Striker Health</b>. Only the First Striker should take losses. All other troops <b>MUST</b> be exactly as calculated.</div></div></div></Modal>
        ...
        <Modal open={resultsOpen} title="📋 Calculated Results" onClose={() => setResultsOpen(false)} theme={theme}>
          {calcOutput ? (
            <>
              ...
              <button onClick={async () => { const list = calcOutput.troops.map(t => `${t.troop} - ${fmtInt(t.required)}`).join("\n"); const ok = await copyToClipboard(list); setCopyNotice(ok ? "✅ Copied!" : "❌ Copy failed"); setTimeout(() => setCopyNotice(""), 1500); }} style={{ width: "100%", padding: "14px", borderRadius: 12, border: "none", background: theme.accent, color: "#000", fontWeight: 800, fontSize: 16, marginBottom: 12, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}><span>📄</span> Copy List to Clipboard</button>
              {copyNotice && <div style={{ textAlign: "center", marginBottom: 16, color: theme.accent, fontWeight: 700 }}>{copyNotice}</div>}
              <div style={{ display: "grid", gap: 8 }}>{calcOutput.troops.map((l, i) => (<div key={i} style={{ padding: "12px 16px", background: theme.cardBg, borderRadius: 12, border: `1px solid ${theme.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: `${theme.shadow}, ${theme.goldGlow}` }}><div style={{ display: "flex", alignItems: "center", gap: 12 }}>{iconSrcForTroop(l.troop) && <img src={iconSrcForTroop(l.troop)} alt={l.troop} width={44} height={44} style={{ borderRadius: 8, border: "1px solid #333" }} />}<span style={{ fontWeight: 700, color: theme.text, fontSize: 16 }}>{l.troop}</span></div><span style={{ fontWeight: 800, color: theme.accent, fontSize: 20 }}>{fmtInt(l.required)}</span></div>))}</div>
            </>
          ) : <div style={{ color: theme.subtext, textAlign: "center", padding: 20 }}>No results to display.</div>}
        </Modal>
      </div>
      <footer style={{ textAlign: "center", padding: "20px", color: theme.subtext, fontSize: 12 }}>© 2026 Game01Master · Non-commercial license</footer>
    </div>
  );
}
